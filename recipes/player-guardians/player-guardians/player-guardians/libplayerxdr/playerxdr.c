...
int xdr_player_chemical_data_t (XDR* xdrs, player_chemical_data_t * msg)
{   if(xdr_u_short(xdrs,&msg->chemical_id) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->concentration) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->temperature) != 1)
    return(0);
  return(1);
...
int 
player_chemical_data_pack(void* buf, size_t buflen, player_chemical_data_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_chemical_data_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_chemical_data_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_chemical_data_t_copy(player_chemical_data_t *dest, const player_chemical_data_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_chemical_data_t));
  return sizeof(player_chemical_data_t);
} 
void player_chemical_data_t_cleanup(const player_chemical_data_t *msg)
{
} 
player_chemical_data_t * player_chemical_data_t_clone(const player_chemical_data_t *msg)
{      
  player_chemical_data_t * clone = malloc(sizeof(player_chemical_data_t));
  if (clone)
    player_chemical_data_t_copy(clone,msg);
  return clone;
}
void player_chemical_data_t_free(player_chemical_data_t *msg)
{      
  player_chemical_data_t_cleanup(msg);
  free(msg);
}
unsigned int player_chemical_data_t_sizeof(player_chemical_data_t *msg)
{
  return sizeof(player_chemical_data_t);
} 
